...
def test_pyproject_exists():
    ...
    pyproject = os.path.join(PLATFORM_ROOT, "pyproject.toml")
    assert os.path.isfile(pyproject), f"Missing: {pyproject}"


def test_dockerfile_exists():
    """Ensure Dockerfile.prod exists."""
    dockerfile = os.path.join(PLATFORM_ROOT, "Dockerfile.prod")
