...
# =============================================================================
# DataOps Platform — Production Deployment Script
# =============================================================================
...
REGISTRY="${DATAOPS_REGISTRY:-ghcr.io/indestructibleorg}"
IMAGE_NAME="${REGISTRY}/dataops-platform"
VERSION="${DATAOPS_VERSION:-$(date -u +%Y%m%d.%H%M%S)}"
IMAGE_TAG="${IMAGE_NAME}:${VERSION}"
NAMESPACE="${DATAOPS_NAMESPACE:-dataops}"
RELEASE_NAME="${DATAOPS_RELEASE:-dataops}"
...
helm upgrade --install "${RELEASE_NAME}" "${PROJECT_ROOT}/helm/dataops" \
...
kubectl -n "${NAMESPACE}" rollout status deployment/dataops-engine --timeout=120s
